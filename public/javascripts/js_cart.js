jQuery(document).ready(function($){
	...
	if( cartWrapper.length > 0 ) {
		...
		var decreaseToCartBtn = $('.decrease-button');
		...
		decreaseToCartBtn.on('click', function(event){
			event.preventDefault();
			decreaseToCart($(this));
		});
	...
	function addProduct(proname,proid,price,proimg) {
		...
		if(quantity==''){
			...
			var productAdded =
			 ('<li class="product  product-'+proid+' "><div class="product-details"><h3><a href="#0">菜名'+proname+',</a></h3><span class="price">'+price+ '</span><div class="actions"><a href="#0" class="delete-item">删除,</a><div class="quantity"><label for="cd-product-'+ proid+'">件数</label>'+select+'.</div></div></div></li>');
	...
	counton.on('click',function(event){
		var my = cartBody.find('ul').eq(-1).text();上
			var str = String(my);
			// var arr = str.split('.');
